...
var builder = primish({
    ...
    if (self.css && !fs.existsSync(self.css)){
      console.log('ERROR:'.red + ' failed to find styles folder ' + self.css);
      error = true;
    }
      ...
      // copy the css files
      if (self.css) {
        fse.copy(self.css, path.resolve(self.options.output, 'css'), function(error){
          if (!error){
            console.log(self.options.ok + ' Copied supplementary stylesheets');
            self.trigger('css');
            self.tasks--;
            self.trigger('taskdone');
          }
          else {
            console.log('Failed to copy supplementary stylesheets files');
            self.trigger('error', error);
          }
        });
      }
